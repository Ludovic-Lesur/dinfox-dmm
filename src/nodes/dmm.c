...
static NODE_status_t _DMM_update_register(uint8_t reg_addr) {
	...
	case DMM_REG_ADDR_CONFIGURATION_0:
		...
		DINFOX_write_field(&(DMM_INTERNAL_REGISTERS[DMM_REG_ADDR_CONFIGURATION_0]), &unused_mask, 0b1, DMM_REG_CONFIGURATION_0_MASK_BMSF);
#else
		DINFOX_write_field(&(DMM_INTERNAL_REGISTERS[DMM_REG_ADDR_CONFIGURATION_0]), &unused_mask, 0b0, DMM_REG_CONFIGURATION_0_MASK_BMSF);
#endif
		break;
...
static NODE_status_t _DMM_check_register(uint8_t reg_addr, uint32_t reg_mask) {
	...
	case DMM_REG_ADDR_CONTROL_1:
		...
		break;
	case DMM_REG_ADDR_CONFIGURATION_1:
		// Nodes scan period.
		if ((reg_mask & DMM_REG_CONFIGURATION_1_MASK_NODES_SCAN_PERIOD) != 0) {
			// Check range.
			duration = DINFOX_get_seconds((DINFOX_time_representation_t) DINFOX_read_field(reg_value, DMM_REG_CONFIGURATION_1_MASK_NODES_SCAN_PERIOD));
			if (duration < DMM_NODES_SCAN_PERIOD_SECONDS_MIN) {
				// Force default value.
				DINFOX_write_field(&(DMM_INTERNAL_REGISTERS[DMM_REG_ADDR_CONTROL_1]), &unused_mask, DINFOX_convert_seconds(DMM_NODES_SCAN_PERIOD_SECONDS_DEFAULT), DMM_REG_CONFIGURATION_1_MASK_NODES_SCAN_PERIOD);
				status = NODE_ERROR_REGISTER_FIELD_RANGE;
				goto errors;
			}
		}
		// Sigfox uplink period.
		if ((reg_mask & DMM_REG_CONFIGURATION_1_MASK_SIGFOX_UL_PERIOD) != 0) {
			// Check range.
			duration = DINFOX_get_seconds((DINFOX_time_representation_t) DINFOX_read_field(reg_value, DMM_REG_CONFIGURATION_1_MASK_SIGFOX_UL_PERIOD));
			if (duration < DMM_SIGFOX_UL_PERIOD_SECONDS_MIN) {
				// Force default value.
				DINFOX_write_field(&(DMM_INTERNAL_REGISTERS[DMM_REG_ADDR_CONTROL_1]), &unused_mask, DINFOX_convert_seconds(DMM_SIGFOX_UL_PERIOD_SECONDS_DEFAULT), DMM_REG_CONFIGURATION_1_MASK_SIGFOX_UL_PERIOD);
				status = NODE_ERROR_REGISTER_FIELD_RANGE;
				goto errors;
			}
		}
		// Sigfox downlink period.
		if ((reg_mask & DMM_REG_CONFIGURATION_1_MASK_SIGFOX_DL_PERIOD) != 0) {
			// Check range.
			duration = DINFOX_get_seconds((DINFOX_time_representation_t) DINFOX_read_field(reg_value, DMM_REG_CONFIGURATION_1_MASK_SIGFOX_DL_PERIOD));
			if (duration < DMM_SIGFOX_DL_PERIOD_SECONDS_MIN) {
				// Force default value.
				DINFOX_write_field(&(DMM_INTERNAL_REGISTERS[DMM_REG_ADDR_CONTROL_1]), &unused_mask, DINFOX_convert_seconds(DMM_SIGFOX_DL_PERIOD_SECONDS_DEFAULT), DMM_REG_CONFIGURATION_1_MASK_SIGFOX_DL_PERIOD);
				status = NODE_ERROR_REGISTER_FIELD_RANGE;
				goto errors;
			}
		}
...
void DMM_init_registers(void) {
	...
	// Read init state.
	_DMM_update_register(DMM_REG_ADDR_CONFIGURATION_0);
	// Load default values.
	DINFOX_write_field(&(DMM_INTERNAL_REGISTERS[DMM_REG_ADDR_CONFIGURATION_1]), &unused_mask, DINFOX_convert_seconds(DMM_NODES_SCAN_PERIOD_SECONDS_DEFAULT), DMM_REG_CONFIGURATION_1_MASK_NODES_SCAN_PERIOD);
	DINFOX_write_field(&(DMM_INTERNAL_REGISTERS[DMM_REG_ADDR_CONFIGURATION_1]), &unused_mask, DINFOX_convert_seconds(DMM_SIGFOX_UL_PERIOD_SECONDS_DEFAULT), DMM_REG_CONFIGURATION_1_MASK_SIGFOX_UL_PERIOD);
	DINFOX_write_field(&(DMM_INTERNAL_REGISTERS[DMM_REG_ADDR_CONFIGURATION_1]), &unused_mask, DINFOX_convert_seconds(DMM_SIGFOX_DL_PERIOD_SECONDS_DEFAULT), DMM_REG_CONFIGURATION_1_MASK_SIGFOX_DL_PERIOD);
	_DMM_reset_analog_data();
}
