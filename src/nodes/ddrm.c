...
static const NODE_line_data_t DDRM_LINE_DATA[DDRM_LINE_DATA_INDEX_LAST - COMMON_LINE_DATA_INDEX_LAST] = {
	{DDRM_REG_ADDR_ANALOG_DATA_1, DDRM_REG_ANALOG_DATA_1_MASK_VIN, "VIN =", STRING_FORMAT_DECIMAL, 0, " V"},
	{DDRM_REG_ADDR_ANALOG_DATA_1, DDRM_REG_ANALOG_DATA_1_MASK_VOUT, "VOUT =", STRING_FORMAT_DECIMAL, 0, " V"},
	{DDRM_REG_ADDR_ANALOG_DATA_2, DDRM_REG_ANALOG_DATA_2_MASK_IOUT, "IOUT =", STRING_FORMAT_DECIMAL, 0, " mA"},
	{DDRM_REG_ADDR_STATUS_CONTROL_1, DDRM_REG_STATUS_CONTROL_1_MASK_DDEN, "DC-DC =", STRING_FORMAT_BOOLEAN, 0, STRING_NULL}
};

static const uint8_t DDRM_REG_ADDR_LIST_SIGFOX_PAYLOAD_MONITORING[] = {
	COMMON_REG_ADDR_ANALOG_DATA_0
};

static const uint8_t DDRM_REG_ADDR_LIST_SIGFOX_PAYLOAD_ELECTRICAL[] = {
...
NODE_status_t DDRM_read_line_data(NODE_line_data_read_t* line_data_read, NODE_access_status_t* read_status) {
	...
	if ((line_data_read -> line_data_index) < COMMON_LINE_DATA_INDEX_LAST) {
		...
		status = COMMON_read_line_data(line_data_read, (uint32_t*) DDRM_REGISTERS);
		...
		// Update register.
		status = XM_read_register((line_data_read -> node_addr), DDRM_LINE_DATA[str_data_idx].reg_addr, (uint32_t*) DDRM_REGISTERS, read_status);
		if (status != NODE_SUCCESS) goto errors;
		// Compute field.
		field_value = DINFOX_read_field(DDRM_REGISTERS[DDRM_LINE_DATA[str_data_idx].reg_addr], DDRM_LINE_DATA[str_data_idx].field_mask);
		// Add data name.
...
/* UPDATE DDRM NODE SIGFOX UPLINK PAYLOAD.
...
NODE_status_t DDRM_build_sigfox_ul_payload(NODE_ul_payload_update_t* ul_payload_update) {
	...
	case NODE_SIGFOX_PAYLOAD_TYPE_STARTUP:
		// Use common format.
		status = COMMON_build_sigfox_payload_startup(ul_payload_update, (uint32_t*) DDRM_REGISTERS);
		if (status != NODE_SUCCESS) goto errors;
		break;
	case NODE_SIGFOX_PAYLOAD_TYPE_MONITORING:
		...
		status = XM_read_registers((ul_payload_update -> node_addr), (uint8_t*) DDRM_REG_ADDR_LIST_SIGFOX_PAYLOAD_MONITORING, sizeof(DDRM_REG_ADDR_LIST_SIGFOX_PAYLOAD_MONITORING), (uint32_t*) DDRM_REGISTERS);
		if (status != NODE_SUCCESS) goto errors;
	...
	case NODE_SIGFOX_PAYLOAD_TYPE_DATA:
		...
		if (status != NODE_SUCCESS) goto errors;
		// Read related registers.
		status = XM_read_registers((ul_payload_update -> node_addr), (uint8_t*) DDRM_REG_ADDR_LIST_SIGFOX_PAYLOAD_ELECTRICAL, sizeof(DDRM_REG_ADDR_LIST_SIGFOX_PAYLOAD_ELECTRICAL), (uint32_t*) DDRM_REGISTERS);
		if (status != NODE_SUCCESS) goto errors;
		// Build data payload.
