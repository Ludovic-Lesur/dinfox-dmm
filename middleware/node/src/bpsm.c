...
NODE_status_t BPSM_write_line_data(NODE_line_data_write_t* line_data_write, NODE_access_status_t* write_status) {
	...
	// Check index.
	if ((line_data_write -> line_data_index) >= BPSM_LINE_DATA_INDEX_LAST) {
...
NODE_status_t BPSM_build_sigfox_ul_payload(NODE_ul_payload_t* node_ul_payload) {
	...
	(node_ul_payload -> node -> radio_transmission_count) = (uint8_t) (((node_ul_payload -> node -> radio_transmission_count) + 1) % (sizeof(BPSM_SIGFOX_UL_PAYLOAD_PATTERN)));
errors:
	return status;
}
