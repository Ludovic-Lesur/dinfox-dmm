...
RADIO_status_t RADIO_LVRM_build_ul_node_payload(RADIO_ul_node_payload_t* node_payload) {
	...
	status = RADIO_COMMON_check_event_driven_payloads(node_payload, (uint32_t*) lvrm_registers);
	...
	switch (RADIO_LVRM_UL_PAYLOAD_PATTERN[node_payload->payload_type_counter]) {
	case RADIO_LVRM_UL_PAYLOAD_TYPE_MONITORING:
		...
		if (access_status.flags == 0) {
			...
			node_status = NODE_read_registers((node_payload->node), (uint8_t*) RADIO_LVRM_REGISTERS_MONITORING, sizeof(RADIO_LVRM_REGISTERS_MONITORING), (uint32_t*) lvrm_registers, &access_status);
			NODE_exit_error(RADIO_ERROR_BASE_NODE);
		}
	...
	case RADIO_LVRM_UL_PAYLOAD_TYPE_ELECTRICAL:
		...
		if (access_status.flags == 0) {
			// Read related registers.
		    node_status = NODE_read_registers((node_payload->node), (uint8_t*) RADIO_LVRM_REGISTERS_ELECTRICAL, sizeof(RADIO_LVRM_REGISTERS_ELECTRICAL), (uint32_t*) lvrm_registers, &access_status);
            NODE_exit_error(RADIO_ERROR_BASE_NODE);
		}
		// Build data payload.
    ...
    node_payload->payload_type_counter = (((node_payload->payload_type_counter) + 1) % sizeof(RADIO_LVRM_UL_PAYLOAD_PATTERN));
errors:
	return status;
}
