...
#define _DINFOX_flush_measurement_value(void) { \
	uint8_t idx = 0; \
	for (idx=0 ; idx<DINFOX_STRING_BUFFER_SIZE ; idx++) data_value_ptr[idx] = STRING_CHAR_NULL; \
	(*data_value_size) = 0; \
}
...
#define _DINFOX_append_string(str) { \
	string_status = STRING_append_string(data_value_ptr, DINFOX_STRING_BUFFER_SIZE, str, data_value_size); \
...
NODE_status_t DINFOX_read_data(RS485_address_t rs485_address, DINFOX_common_data_index_t data_index, char_t* data_value_ptr, uint8_t* data_value_size) {
	...
	// Check parameters.
	if ((data_value_ptr == NULL) || (data_value_size == NULL)) {
